...
namespace LoanShark.Repository.SocialRepository
{
    using System;
    ...
    public class Repository : IRepository
    {
        ...
        public User? GetUserById(int userID)
        {
            ...
            if (dataTable.Rows.Count > 0)
            {
                ...
                string phone = row["phone_number"]?.ToString() ?? string.Empty;
                PhoneNumber phoneNumber = new PhoneNumber(phone);
                int reportedCount = Convert.ToInt32(row["reported_count"]);
        ...
        public Chat? GetChatById(int chatID)
        {
            ...
            {
                DataRow row = dataTable.Rows[0];
                int id = Convert.ToInt32(row["id_chat"]);
                string chatName = row["chat_name"]?.ToString() ?? string.Empty;
        ...
        public List<NotificationModel> GetNotifications(int userID)
        {
            ...
            DataTable dataTable = this.dbConnection.ExecuteReader("SELECT * FROM Notifications WHERE id_user = @id_user ORDER BY Timestamp DESC", parameters, false);
            List<NotificationModel> notifications = new List<NotificationModel>();

            foreach (DataRow row in dataTable.Rows)
            {
                int notificationID = Convert.ToInt32(row["id_notification"]);
                DateTime timestamp = Convert.ToDateTime(row["timestamp"]);
                string content = row["content"]?.ToString() ?? string.Empty;
                int userReceiverID = Convert.ToInt32(row["id_user"]);
        ...
        public List<User> GetUserFriendsList(int userId)
        {
            ...
            foreach (DataRow row in dataTable1.Rows)
            {
                ...
                {
                    friendIds.Add(Convert.ToInt32(row["id_friend"]));
                }
            }
        ...
        public List<Chat> GetChatsList()
        {
            ...
            foreach (DataRow row in dataTable.Rows)
            {
                ...
                foreach (DataRow row1 in dataTable1.Rows)
                {
                    if (chatID == Convert.ToInt32(row1["id_chat"]))
                    {
                        participantsIDs.Add(Convert.ToInt32(row1["id_user"]));
                    }
                }
        ...
        public List<User> GetChatParticipants(int chatID)
        {
            ...
            DataTable dataTable = this.dbConnection.ExecuteReader("SELECT * FROM Chat_Participants WHERE id_chat = @ChatID", parameters, false);
            List<int> userIds = new List<int>();
            foreach (DataRow row in dataTable.Rows)
            {
                userIds.Add(Convert.ToInt32(row["id_user"]));
            }
        ...
        public List<Message> GetMessagesList()
        {
            ...
            foreach (DataRow row in dataTable.Rows)
            {
                int messageID = Convert.ToInt32(row["id_message"]);
                int typeID = Convert.ToInt32(row["id_type"]);
                int userID = Convert.ToInt32(row["id_user"]);
                int chatID = Convert.ToInt32(row["id_chat"]);
                DateTime timestamp = Convert.ToDateTime(row["timestamp"]);
                string content = row["content"]?.ToString() ?? string.Empty;
                ...
                {
                    if (Convert.ToInt32(row1["id_message"]) == messageID)
                    {
        ...
        public List<int> GetFriendsIDs(int userID)
        {
            ...
            string query = "SELECT id_friend FROM Friends WHERE userid = @UserID";
            SqlParameter[] parameters =
            {
        new SqlParameter("@UserID", userID),
    };
            ...
            foreach (DataRow row in dataTable.Rows)
            {
                ...
                friends.Add(Convert.ToInt32(row["id_friend"]));
            }
        ...
        public List<Post> GetFeedPostsList()
        {
            ...
            foreach (DataRow row in dataTable.Rows)
            {
                int postID = Convert.ToInt32(row["id_post"]);
        ...
        public List<int> GetChatsIDs(int userID)
        {
            ...
            foreach (DataRow row in dataTable.Rows)
            {
                chats.Add(Convert.ToInt32(row["id_chat"]));
            }
        ...
        public List<int> GetChatParticipantsIDs(int chatID)
        {
            ...
            foreach (DataRow row in dataTable.Rows)
            {
                if (Convert.ToInt32(row["id_chat"]) == chatID)
                {
                    participants.Add(Convert.ToInt32(row["id_user"]));
                }
            }
